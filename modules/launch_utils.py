...
def prepare_environment():
    ...
    if not args.skip_torch_cuda_test:
        has_cuda = check_run_python("import torch; assert torch.cuda.is_available()")
        has_musa = check_run_python("import torch_musa; assert torch_musa.is_available()")
        if not (has_cuda or has_musa):
            raise RuntimeError(
                'Your device does not support the current version of Torch/CUDA or MUSA! Consider download another version: \n'
